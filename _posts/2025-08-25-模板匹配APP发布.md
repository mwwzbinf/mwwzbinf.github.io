...
APP的界面设计如下：上方和下方的**加载图片**按钮分别用于导入模板图像和待匹配图像。图片加载后支持多种手势操作——**单指拖动**可平移图像，**双指捏合**可缩放图像，**双击屏幕**可自适应显示图像。**创建模板**无需任何参数。**寻找模板** 提供忽略极性、多尺度匹配、亚像素匹配三项可选设置。操作后亚像素选项右侧显示耗时，轻击该区域可切换查看匹配结果数值，再次点击返回图像模式。
