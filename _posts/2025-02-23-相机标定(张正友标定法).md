...
\end{bmatrix}
$$
其中相机内参矩阵$\mathbf{K}=\begin{bmatrix}
\alpha& \gamma&u_0\\
0&\beta&v_0\\
0&0&1
\end{bmatrix}$，$\alpha = s_x f$，$\beta = s_y f$，$\gamma$为倾斜因子，通常$\gamma = 0$。

**简化变换（棋盘格平面$Z_w = 0$）**

在张正友标定法中，使用棋盘格作为标定物，将棋盘格平面置于世界坐标系的$Z_w = 0$平面上。此时，$\begin{bmatrix}
\mathbf{X}_w\\
1
\end{bmatrix}=\begin{bmatrix}
X_w\\
...
对于每一幅棋盘格图像，已知棋盘格角点的世界坐标$\mathbf{X}_w = [X_w, Y_w, 1]^T$和对应的像素坐标$\mathbf{x}=[u, v, 1]^T$，可以通过最小二乘法求解单应性矩阵$\mathbf{H}=(h_{ij})$。
...
因为旋转矩阵$\mathbf{R}$的列向量是正交的，即$\mathbf{r}_1^T\mathbf{r}_2 = 0$且$\|\mathbf{r}_1\|=\|\mathbf{r}_2\| = 1$，所以有：
$$
...
定义向量$\mathbf{b}=[B_{11},B_{12},B_{22},B_{13},B_{23},B_{33}]^T$，对于单应性矩阵的列向量$\mathbf{h}_i=(h_{i1},h_{i2},h_{i3})^T$，可以得到：
$$
\mathbf{h}_i^T\mathbf{B}\mathbf{h}_j=\begin{bmatrix}
...
$$
\begin{cases}
\mathbf{h}_1^T\mathbf{B}\mathbf{h}_2 = 0\\
\mathbf{h}_1^T\mathbf{B}\mathbf{h}_1-\mathbf{h}_2^T\mathbf{B}\mathbf{h}_2 = 0
\end{cases}
$$
